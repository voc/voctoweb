namespace :media do
  ...
  desc "import videos from one folder into the database with less auto deduction. each line of the input-file should contain a slug-refix for finding the event and a filename. uses ffprobe"
  task :import_folder_with_mapping_ffprobe => :environment do |t,args|
